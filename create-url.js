...
const MESSAGES = {
  ja: {
    ...
    design:'パラメータを入力', profile:'テンプレート', save_name:'名前', destination:'遷移先URL', hash:'ハッシュ', parameter:'パラメータ', value:'値', key:'キー', note:'ノート', move_up:'上へ移動', move_down:'下へ移動', delete:'削除', add_param:'＋ パラメータを追加', no_params:'パラメータは未入力', no_params_hint:'キーと値を追加',
    complete_url:'生成結果', preview_empty:'遷移先URLを入力すると表示', copy:'URLをコピー', save:'保存する', save_hint:'保存方法を選択', check_before:'入力を確認', quality:'確認事項',
    saved_urls:'保存したURL', library_hint:'検索、編集、出力、バックアップ', export_csv:'CSVを出力', backup:'バックアップを保存', restore:'バックアップを読み込む', delete_all:'すべて削除', search:'名前、ノート、URLで検索', no_saved:'保存したURLはない。', edit:'編集', delete:'削除', updated:'更新',
    matrix_title:'複数のURLをまとめて作る', matrix_hint:'現在のURLをもとに最大200件作成', replace_key:'置き換えるパラメータ', values:'値（改行またはカンマ区切り）', remove_axis:'このルールを削除', add_axis:'＋ 置換ルールを追加', matrix_empty:'URLと置換ルールを入力', matrix_limit:'先頭200件のみ表示・出力',
    my_profiles:'テンプレート', profile_hint:'よく使うパラメータ構成を保存', profile_name:'例: 自社メール計測', save_profile:'テンプレートを保存', profile_info:'パラメータ名のみ保存', no_profiles:'テンプレートはない。', use:'使う', update:'現在の構成で更新',
    save_question:'保存先を選ぶ', save_description:'あとで編集する場合に保存', device_save:'この端末に保存', device_save_hint:'この端末であとから編集', account_save:'アカウント保存（準備中）', account_save_hint:'複数端末・共有に対応予定', close:'閉じる',
    profile_generic:'自由入力', profile_generic_desc:'任意のキーと値を追加', profile_ga4:'Webトラッキング（UTM）', profile_ga4_desc:'GA4などで使う一般的なUTMパラメータ', profile_adobe:'Adobe Tracking Code', profile_adobe_desc:'Adobe Analyticsなどの計測用', profile_campaign:'広告・CRM計測', profile_campaign_desc:'広告・CRM連携向けの雛形',
  ...
  en: {
    ...
    design:'Enter parameters', profile:'Template', save_name:'Name', destination:'Destination URL', hash:'Hash', parameter:'Parameter', value:'Value', key:'Key', note:'Note', move_up:'Move up', move_down:'Move down', delete:'Delete', add_param:'＋ Add parameter', no_params:'No parameters yet', no_params_hint:'Add a key and value',
    complete_url:'Generated URL', preview_empty:'Enter a destination URL to preview', copy:'Copy URL', save:'Save', save_hint:'Choose where to save', check_before:'Check your input', quality:'Notes',
    saved_urls:'Saved URLs', library_hint:'Search, edit, export, and back up', export_csv:'Export CSV', backup:'Save backup', restore:'Load backup', delete_all:'Delete all', search:'Search name, note, or URL', no_saved:'No saved URLs.', edit:'Edit', delete:'Delete', updated:'Updated',
    matrix_title:'Create multiple URLs', matrix_hint:'Create up to 200 URLs from the current URL', replace_key:'Parameter to replace', values:'Values (new line or comma)', remove_axis:'Remove rule', add_axis:'＋ Add replacement rule', matrix_empty:'Enter a URL and replacement rules', matrix_limit:'Only the first 200 results are shown and exported',
    my_profiles:'Templates', profile_hint:'Save parameter structures you use often', profile_name:'Example: Company email tracking', save_profile:'Save template', profile_info:'Only parameter names are saved', no_profiles:'No templates yet.', use:'Use', update:'Update from current structure',
    save_question:'Choose where to save', save_description:'Save to edit later', device_save:'Save on this device', device_save_hint:'Edit later on this device', account_save:'Account save (coming soon)', account_save_hint:'Multi-device access and sharing are coming soon', close:'Close',
    profile_generic:'Free entry', profile_generic_desc:'Add any key and value', profile_ga4:'Web tracking (UTM)', profile_ga4_desc:'Common UTM parameters for GA4 and more', profile_adobe:'Adobe Tracking Code', profile_adobe_desc:'For Adobe Analytics and more', profile_campaign:'Ads & CRM tracking', profile_campaign_desc:'A starter for ads and CRM',
...
const app = Vue.createApp({
  ...
  methods: {
    ...
    documentUrl(item) { return buildUrl(item); },
    ...
    <main class="app-shell">
      ...
      <section v-if="activeTab==='builder'" class="workspace">
        <div class="builder-main">
          <section class="start-panel" aria-labelledby="start-title"><div><p class="eyebrow">{{ t('start') }}</p><h2 id="start-title">{{ t('what') }}</h2><p>{{ t('start_hint') }}</p></div><div class="start-grid"><button type="button" class="start-card primary-start" @click="startWithProfile('ga4')"><strong>{{ t('web_start') }}</strong><span>{{ t('web_hint') }}</span></button><button type="button" class="start-card" @click="startWithProfile('campaign')"><strong>{{ t('campaign_start') }}</strong><span>{{ t('campaign_hint') }}</span></button><button type="button" class="start-card" @click="focusPaste"><strong>{{ t('edit_url') }}</strong><span>{{ t('edit_hint') }}</span></button><button type="button" class="start-card" @click="startWithProfile('generic')"><strong>{{ t('scratch') }}</strong><span>{{ t('scratch_hint') }}</span></button></div><button type="button" class="library-link" @click="openLibrary">{{ t('open_saved') }}</button></section>
          <section class="panel intake"><div class="section-heading"><div><h2>{{ t('inspector_title') }}</h2></div><button type="button" class="quiet" @click="newDocument">{{ t('reset') }}</button></div><div class="paste-row"><input id="paste-url" v-model="pasteUrl" type="url" inputmode="url" placeholder="https://example.com/page?existing=value#section" :aria-label="t('parse')"><button type="button" @click="parsePastedUrl">{{ t('parse') }}</button></div><p class="hint">{{ t('parse_hint') }}</p></section>
          <section class="panel"><div class="section-heading"><div><h2>{{ t('design') }}</h2></div><span class="profile-pill">{{ profileLabel(activeProfile) }}</span></div>
            ...
            <div v-for="(param,index) in document.params" :key="param.id" class="parameter-row"><div><label class="sr-only" :for="'key-'+param.id">{{ t('parameter') }}</label><input :id="'key-'+param.id" v-model="param.key" :placeholder="t('key')"><small>{{ displayLabel(param) }}</small></div><div><label class="sr-only" :for="'value-'+param.id">{{ t('value') }}</label><input :id="'value-'+param.id" v-model="param.value" @change="normalizeParameter(param)" :list="fieldFor(param.key)?.choices ? 'choices-'+param.id : null" :placeholder="t('value')"><datalist v-if="fieldFor(param.key)?.choices" :id="'choices-'+param.id"><option v-for="choice in fieldFor(param.key).choices" :key="choice" :value="choice"></option></datalist></div><div class="row-actions"><button type="button" class="icon-button" :disabled="index===0" @click="moveParameter(index,-1)" :aria-label="t('move_up')">↑</button><button type="button" class="icon-button" :disabled="index===document.params.length-1" @click="moveParameter(index,1)" :aria-label="t('move_down')">↓</button><button type="button" class="icon-button danger" @click="removeParameter(param.id)" :aria-label="t('delete')">×</button></div></div>
