...
- **ゼロから作る**: 独自のキー・値を自由に追加します。
- **GA4のURLを作る**: UTMの入力欄から始めます。
- **Adobe用に作る**: `cid` を含むTracking Codeの入力欄から始めます。
- **既存URLを編集する**: URLを貼り付け、既存パラメータを分解して編集します。

その後、遷移先URLとパラメータを設定し、品質チェックを確認してURLをコピーします。必要ならノートを残して「この端末に保存」を選ぶと、ログインなしで後から編集できます。
